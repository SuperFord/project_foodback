...
// Storage configuration - ปรับปรุงให้สร้างโฟลเดอร์อัตโนมัติ
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    // ตรวจสอบว่าเป็นการอัปโหลดสลิปการจ่ายเงินหรือไม่
    let uploadDir = "./uploads/"

    // ถ้าเป็น payment slip ให้ไปโฟลเดอร์ Payment slip
    if (req.path.includes("payment-slip")) {
      uploadDir = "./uploads/Payment slip/"
    }
...
app.get("/api/settings/qr-payment", async (req, res) => {
  try {
    const result = await pool.query(
...
app.put("/api/settings/qr-payment", async (req, res) => {
...
app.put("/api/settings/reservation-window", async (req, res) => {
...
app.post("/api/upload-payment-slip", authenticateToken, upload.single("paymentSlip"), async (req, res) => {
  try {
    ...
    // แก้ไข path ให้ชี้ไปที่โฟลเดอร์ Payment slip
    const slipPath = `/uploads/Payment slip/${req.file.filename}`
    const parsedReservationData = JSON.parse(reservationData)

    console.log("Payment slip saved to:", slipPath)

    const client = await pool.connect()
    try {
      await client.query("BEGIN")

      // บันทึกข้อมูลสลิปการจ่ายเงิน
...
app.get("/api/payment-slips", async (req, res) => {
  try {
...
app.put("/api/payment-slips/:id/status", async (req, res) => {
  const client = await pool.connect()
  try {
    const { id } = req.params
    ...
    if (status === "rejected" && reservationData) {
      ...
      if (email) {
        // ค้นหาการจองล่าสุดด้วย email อย่างเดียว (ไม่ต้องใช้วันที่)
        console.log("🔍 ค้นหาการจองล่าสุดด้วย email อย่างเดียว")
        const reservationResult = await client.query(
          "SELECT id, setable, date FROM reservations WHERE email = $1 ORDER BY id DESC LIMIT 1",
          [email],
        )
        ...
        if (reservationResult.rows.length > 0) {
          ...
          // อัปเดตสถานะโต๊ะเป็นว่าง (status = 1)
          let updatedTables = 0
          for (const table of tables) {
            ...
            if (updateTableResult.rowCount > 0) {
              console.log(`✅ ปรับสถานะโต๊ะสำเร็จ: ${table}`)
              updatedTables++
            } else {
          ...
          // ลบข้อมูลใน reservation_foods (อิง foreign key)
          await client.query("DELETE FROM reservation_foods WHERE reservation_id = $1", [reservation.id])

          // ลบข้อมูลการจอง
          const deleteResult = await client.query("DELETE FROM reservations WHERE id = $1", [reservation.id])

          if (deleteResult.rowCount > 0) {
            console.log(`✅ ยกเลิกการจองสำเร็จสำหรับ: ${email}`)
            console.log(`📊 สรุป: อัปเดตโต๊ะ ${updatedTables}/${tables.length} โต๊ะ`)
          }
        } else {
...
//อัปเดทสถานะเมนู
app.put("/api/menus/:id/status", async (req, res) => {
  const { id } = req.params
...
app.post("/api/table_map", upload.single("image"), async (req, res) => {
  const imagePath = req.file ? `/uploads/${req.file.filename}` : null // ใช้ filename ที่มี timestamp
  ...
  const client = await pool.connect()
  try {
    // บันทึกไฟล์รูปภาพลงใน table_map
...
app.post("/api/table_layout", upload.none(), async (req, res) => {
...
app.put("/api/table_layout/:id/status", async (req, res) => {
...
app.get("/api/table_today", async (req, res) => {
  const client = await pool.connect()
  try {
    const query = "SELECT tnumber, tname , status FROM table_layout ORDER BY tnumber ASC"
    const result = await client.query(query)
...
app.get("/api/reservation_by_table", async (req, res) => {
  try {
...
app.post("/api/delete_table_data", async (req, res) => {
...
app.put("/api/table_layout/time-required", async (req, res) => {
...
app.get("/api/restaurant/history", async (req, res) => {
  try {
    // ดึงการจองทั้งหมด
